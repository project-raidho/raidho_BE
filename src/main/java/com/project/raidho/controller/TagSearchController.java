...
public class TagSearchController {
    ...
    public ResponseEntity<?> MeetingTagSeatch(@PathVariable(value = "meetingtag") String tag,
                                              @RequestParam (value = "page",defaultValue = "0")int page,
                                              @RequestParam (value = "size",defaultValue = "5")int size,
                                              @AuthenticationPrincipal UserDetails userDetails) throws ParseException {
        return ResponseEntity.ok().body(tagSearchService.meetingTagSeatch(page, size, tag, userDetails));
    }
